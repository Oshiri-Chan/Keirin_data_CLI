...
class WinticketStep4API:
    """
    ステップ4: Winticketのオッズ情報を取得するAPI
    """
    ...
    # APIエンドポイント
    ENDPOINTS = {"race_odds": "/races/{race_id}/odds"}
    ...
    def get_race_odds(self, race_id):
        ...
        Returns:
            dict or None: オッズ情報、取得失敗時はNone
        """
        # オッズAPIエンドポイント
        endpoint = self.ENDPOINTS["race_odds"].format(race_id=race_id)

        # APIリクエスト
        self.logger.info(f"レース {race_id} のオッズ情報を取得します")

        try:
            odds_data = self._make_api_request(endpoint)

            if not odds_data:
                self.logger.warning(f"レース {race_id} のオッズ情報取得に失敗しました")
                return None

            # オッズ情報の種類を確認
            odds_types = []
            for key in odds_data:
                if isinstance(odds_data[key], dict) and odds_data[key]:
                    odds_types.append(key)

            self.logger.info(
                f"レース {race_id} のオッズ情報を取得しました（種類: {', '.join(odds_types)}）"
            )
            return odds_data
